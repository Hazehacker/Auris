...
| ----------- | ----------------------- | ------------------------------------------------------------ | -------- |
| id          | bigint                  | 主键                                                         |          |
| title       | VARCHAR(255)            | 歌曲标题（可编辑）                                           |          |
| artist      | VARCHAR(255)            | 歌手（可编辑）                                               |          |
| album       | VARCHAR(255)            | 专辑（可编辑）                                               |          |
| duration    | INTEGER                 | 时长（秒）                                                   |          |
| file_path   | VARCHAR(2048)           | 本地文件路径 **或** 在线 URL（如 `https://example.com/song.mp3`） |          |
| cover_url   | VARCHAR(512)            | 封面图片 URL 或 Base64/Data URL（可为空）                    |          |
| source_type | ENUM('local', 'online') | 来源类型                                                     | 暂时引入 |
| create_time | DATETIME                | 上传/添加时间                                                |          |
| user_id     | UUID / INT              | 关联用户（若支持多用户）                                     |          |
...
| 字段名      | 类型     | 说明                               | 备注     |
| ----------- | -------- | ---------------------------------- | -------- |
| playlist_id | bigint   | 外键 → `playlists.id`              |          |
| track_id    | bigint   | 外键 → `tracks.id`                 |          |
